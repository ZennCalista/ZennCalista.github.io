...
$proposal_title = isset($_POST['proposal_title']) ? $_POST['proposal_title'] : null;
$proposal_description = isset($_POST['proposal_description']) ? $_POST['proposal_description'] : null;

if (!isset($_FILES['document_file'])) {
    die('No file uploaded');
}

// Handle proposal submission differently
if ($document_type === 'proposal') {
    // This is a new proposal submission
    if (empty($proposal_title)) {
        die('Proposal title is required for proposal submissions');
    }

    // Create proposal record
    $proposal_sql = "INSERT INTO program_proposals (faculty_id, proposal_title, description, status, submitted_at)
                     VALUES (?, ?, ?, 'pending', NOW())";
    $proposal_stmt = $conn->prepare($proposal_sql);
    $proposal_stmt->bind_param('iss', $faculty_id, $proposal_title, $proposal_description);
    if (!$proposal_stmt->execute()) {
        die('Failed to create proposal: ' . $conn->error);
    }
    ...
    if (!$program_id) {
        die('Program selection is required for this document type');
    }
    $proposal_id = null;
}
