...
<b>Faculty:</b> ${program.faculty_name}
  ...
  function showMyProgramsTab(filter) {
    ...
    if (filter === 'active') {
      ...
      // Show both pending and rejected applications in the Pending tab
      filteredPrograms = allMyPrograms.filter(p => 
        p.enrollment_status === 'pending' || p.enrollment_status === 'rejected'
      );
  ...
  function createEnhancedMyProgramCard(program, type) {
    ...
    if (type === 'active') {
      ...
      let rejectionHtml = '';
      if (program.enrollment_status === 'rejected' && program.reject_reason) {
        rejectionHtml = `
          <div class="rejection-reason" style="color:#dc3545; margin-top:10px;">
            <i class="fas fa-times-circle"></i>
            <b>Application Rejected:</b> ${program.reject_reason}
          </div>
        `;
      }
      actionButtons = `
